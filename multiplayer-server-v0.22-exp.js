...
/**
 * Power Rangers RPG Character Builder — v0.22-exp GM check-request room server
...
const MAX_PAYLOAD = Number(process.env.MAX_PAYLOAD || 2 * 1024 * 1024);
...
function publicRoom(room, viewerId) {
  ...
  return {
    ...
    encounter, lifecycle: room.lifecycle, revision: room.revision, createdAt: room.createdAt, updatedAt: room.updatedAt
  };
}
...
const server = http.createServer((req, res) => {
  if (req.url === '/health') {
    ...
    res.end(JSON.stringify({ ok: true, protocol: PROTOCOL, rooms: rooms.size, version: '0.22-exp' }));
    return;
  }
...
server.listen(PORT, HOST, () => console.log(`Power Rangers RPG v0.22-exp GM room server listening on ws://${HOST}:${PORT}`));
