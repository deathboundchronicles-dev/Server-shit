...
/**
 * Power Rangers RPG Character Builder — v0.33-exp GM Session Flexibility & Reveal Board room server
...
function effectiveConditions(v){const out=new Set(cleanConditions(v));if(out.has('Asleep')){out.add('Prone');out.add('Unconscious')}if(out.has('Defeated'))out.add('Prone');if(out.has('Restrained'))out.add('Immobilized');return [...out];}
...
function tacticalTokenInfo(room,id){if(id.startsWith('pc:')){const clientId=id.slice(3),m=room.members.get(clientId),p=m?.player?.role==='gm'?m.player.dmpc:m?.player;if(!p)return null;return{id,kind:'player',clientId,size:'Common',conditions:p.conditions||[],movement:+p.movement||0,mobility:cleanMobility(p.mobility),side:'player'};}if(id.startsWith('zord:')){const clientId=id.slice(5),m=room.members.get(clientId),p=m?.player?.role==='gm'?m.player.dmpc:m?.player,z=p?.teamSnapshot?.zord;if(!p||!z)return null;return{id,kind:'zord',clientId,size:z.stats?.size||'Huge',conditions:[],movement:parseFloat(String(z.stats?.ground??z.stats?.movement??0))||0,mobility:cleanMobility({}),side:'player'};}const a=(room.encounter?.actors||[]).find(x=>x.id===id&&x.kind==='npc');if(!a)return null;const th=a.threat||{};const mv=String(th.movement||'');const ground=mv.match(/(?:ground(?: movement)?\s*)?(\d+(?:\.\d+)?)\s*ft/i),any=mv.match(/(\d+(?:\.\d+)?)\s*ft/i);return{id,kind:'npc',clientId:'',size:th.size||'Common',conditions:a.conditions||[],movement:+(ground?.[1]||any?.[1]||0),mobility:threatMobility(th),side:'npc',threat:th};}
function tacticalTokens(room){const out=[];for(const m of room.members.values()){const p=m.player.role==='gm'?m.player.dmpc:m.player;if(!p)continue;const pc=tacticalTokenInfo(room,`pc:${p.clientId}`);if(pc)out.push(pc);if(p.zordPresence&&!['lair','recalled','—'].includes(p.zordPresence)&&p.teamSnapshot?.zord?.configured){const z=tacticalTokenInfo(room,`zord:${p.clientId}`);if(z)out.push(z);}}for(const a of room.encounter?.actors||[])if(a.kind==='npc'){const t=tacticalTokenInfo(room,a.id);if(t)out.push(t);}return out;}
...
function movementLocked(tok){return effectiveConditions(tok.conditions||[]).some(x=>['Grappled','Immobilized','Stunned','Unconscious'].includes(x));}
...
function roughIntervals(room,a,b,tok){const map=room.tactical?.map||blankTactical().map,tr=tacticalTokenRect(room,tok,a),hw=tr.w/2,hh=tr.h/2,ints=[];for(const z of map.terrain||[])if(z.terrainType==='difficult'){const r={left:Math.min(z.x1,z.x2)-hw,right:Math.max(z.x1,z.x2)+hw,top:Math.min(z.y1,z.y2)-hh,bottom:Math.max(z.y1,z.y2)+hh};ints.push(segmentRectInterval(a,b,r));}const moverI=sizeOrderIndex(tok.size),g=map.gridSize||64,positions=room.tactical?.positions||{};for(const o of tacticalTokens(room)){if(o.id===tok.id||tokensAllied(tok,o)||moverI-sizeOrderIndex(o.size)>=2)continue;const or=tacticalTokenRect(room,o,positions[o.id]||defaultTokenPos(room,o)),r={left:or.left-g-hw,right:or.right+g+hw,top:or.top-g-hh,bottom:or.bottom+g+hh};ints.push(segmentRectInterval(a,b,r));}if(effectiveConditions(tok.conditions||[]).includes('Blinded'))ints.push([0,1]);return mergeIntervals(ints);}
function tacticalMoveCost(room,a,b,tok){const map=room.tactical?.map||blankTactical().map,base=gridDistance(a,b,map);if(!base)return 0;const rough=roughIntervals(room,a,b,tok).reduce((n,x)=>n+(x[1]-x[0]),0),prone=effectiveConditions(tok.conditions||[]).includes('Prone')?1:0;return base*(1+Math.min(1,rough)+prone);}
...
function cleanDmpc(p, clientId) {
  if (!p || typeof p !== 'object' || (!p.characterId && !p.name)) return null;
  const health = p.health && typeof p.health === 'object' ? p.health : {};
  const power = p.power && typeof p.power === 'object' ? p.power : {};
  return {
    clientId, role: 'dmpc', dmpc: true,
    characterId: cleanText(p.characterId, 160),
    name: cleanText(p.name || 'GM Ranger', 120), ranger: cleanText(p.ranger || '—', 40), level: clampNumber(p.level || 1, 1, 20, 1),
    rulesContext: cleanRulesContext(p.rulesContext), rulesFingerprint: cleanText(p.rulesFingerprint, 1000),
    ...
    morphed: !!p.morphed, movement: clampNumber(p.movement || 0, 0, 9999, 0), mobility: cleanMobility(p.mobility),
    conditions: cleanConditions(p.conditions), initiative: nullableNumber(p.initiative), turnActive: !!p.turnActive, defending: !!p.defending,
    zordPresence: cleanText(p.zordPresence || 'lair', 40),
    art: { civilian: cleanAssetId(p.art?.civilian||''), morphed: cleanAssetId(p.art?.morphed||''), zord: cleanAssetId(p.art?.zord||'') },
    activity: cleanText(p.activity || 'Ready', 500), teamSnapshot: cleanTeamSnapshot(p.teamSnapshot), gmSheet: cleanGmSheet(p.gmSheet), updatedAt: now()
  };
}
function cleanMember(p, clientId, role) {
  ...
  return {
    ...
    defending: role === 'gm' ? false : !!p.defending,
    ...
    dmpc: role === 'gm' ? cleanDmpc(p.dmpc, clientId) : null,
    updatedAt: now()
  };
}
...
function playerActorFromMember(member, existing = null, preserveInitiative = false) {
  const p = member.player;
  const gmOwned = existing?._initiativeOwner === 'gm';
  return {
    ...
    name: p.name,
    ranger: p.ranger,
    initiative: (preserveInitiative || gmOwned) && existing ? existing.initiative : (p.initiative ?? existing?.initiative ?? null),
    _initiativeOwner: gmOwned ? 'gm' : 'player',
    health: null,
    conditions: [],
    notes: ''
  };
...
function syncEncounterPlayers(room) {
  ...
  const activeActorId = currentActor(encounter)?.id || '';
  const oldTurnIndex = encounter.turnIndex;
  ...
  for (const member of room.members.values()) {
    if (member.player.role === 'gm') {
      if (member.player.dmpc) players.push(playerActorFromMember({ player: member.player.dmpc }, oldPlayers.get(member.player.clientId), !!encounter.active));
      continue;
    }
    players.push(playerActorFromMember(member, oldPlayers.get(member.player.clientId), !!encounter.active));
  }
  encounter.actors = [...players, ...npcs].slice(0, MAX_ACTORS);
  if (encounter.active && activeActorId) {
    const preserved = encounter.actors.findIndex(a => a.id === activeActorId);
    encounter.turnIndex = preserved >= 0 ? preserved : (encounter.actors.length ? Math.min(Math.max(0, oldTurnIndex), encounter.actors.length - 1) : -1);
    if (!encounter.actors.length) encounter.active = false;
  } else if (encounter.turnIndex >= encounter.actors.length) encounter.turnIndex = encounter.actors.length ? encounter.actors.length - 1 : -1;
...
function publicRoom(room, viewerId) {
  ...
  for (const [id, m] of room.members) {
    ...
    if (p.dmpc) p.dmpc = { ...p.dmpc, connected: !!m.conn, dmpc: true };
    if (!viewerIsGm) { delete p.gmSheet; if (p.dmpc) delete p.dmpc.gmSheet; }
    members[id] = p;
  }
  const encounter = jsonClone(room.encounter, blankEncounter());
  if (encounter?.actors) for (const a of encounter.actors) { delete a._initiativeOwner; if (!viewerIsGm && a.kind === 'npc') { delete a.threat; delete a.templateId; } }
  return {
    ...
    encounter, tactical: jsonClone(room.tactical || blankTactical(), blankTactical()), lifecycle: room.lifecycle, reveal: jsonClone(room.reveal || {active:false,assetId:'',name:'',serial:0,updatedAt:now()}, {active:false,assetId:'',name:'',serial:0,updatedAt:now()}), revision: room.revision, createdAt: room.createdAt, updatedAt: room.updatedAt
  };
}
function touch(room) { room.revision += 1; room.updatedAt = now(); }
function broadcast(room) {
  const gmFrame = makeFrame(0x1, Buffer.from(JSON.stringify({ type: 'room_state', room: publicRoom(room, room.ownerId) })));
  const playerFrame = makeFrame(0x1, Buffer.from(JSON.stringify({ type: 'room_state', room: publicRoom(room, '') })));
  for (const [id, member] of room.members) if (member.conn) member.conn.sendFrame(id === room.ownerId ? gmFrame : playerFrame);
}
...
function removeMember(room, clientId, announce = true) {
  ...
  if (clientId === room.ownerId && room.reveal?.active) { const old=room.reveal.assetId; room.reveal={active:false,assetId:'',name:'',serial:(room.reveal.serial||0)+1,updatedAt:now()}; if(old&&room.assets?.delete(old))broadcastAssetRemove(room,old); }
  // Player-owned art is session cache, not durable room state. Drop it once the
  // disconnect grace expires so long-running rooms cannot accumulate ghost assets.
  for (const id of [...(room.assets?.keys?.() || [])]) if (id.startsWith(`pc:${clientId}:`)) { room.assets.delete(id); broadcastAssetRemove(room, id); }
  // A movement check cannot outlive its target Ranger or the GM authority that
  // initiated it. Temporary disconnects keep checks during the grace window;
  // permanent member removal cancels the now-orphaned asynchronous operation.
  for (const [rid,pending] of roomPendingMoves(room)) if (pending.targetClientId === clientId || clientId === room.ownerId) {
    roomPendingMoves(room).delete(rid);
    const message=clientId===room.ownerId?'Pending movement canceled because the GM left the room.':'Pending movement canceled because the Ranger left the room.';
    sendMoveGateResult(room,pending.targetClientId,message,{success:false});
  }
  for (const [key,pending] of roomPendingChecks(room)) if (pending.targetClientId === clientId || clientId === room.ownerId) roomPendingChecks(room).delete(key);
...
function attach(conn, room, clientId, player, role) {
  ...
  replayPendingMoveGates(room, clientId, conn);
  replayPendingChecks(room, clientId, conn);
}
function createRoom(conn, msg, clientId) {
  ...
  const room = {
    ...
    assets: new Map(),
    reveal: { active: false, assetId: '', name: '', serial: 0, updatedAt: now() },
...
function roomPendingChecks(room){if(!(room.pendingChecks instanceof Map))room.pendingChecks=new Map();return room.pendingChecks;}
function pendingCheckKey(requestId,targetId){return `${cleanText(requestId||'',180)}:${safeClientId(targetId)}`;}
function sendPendingCheckCommand(conn,pending){if(!conn||!pending)return;conn.sendJSON({type:'gm_command',command:{id:pending.requestId,type:'check_request',targetId:pending.targetClientId,skill:pending.skill,dc:pending.dc,note:pending.note,gmName:'GM',at:pending.at||now()}});}
function replayPendingChecks(room,clientId,conn){if(!conn)return;for(const pending of roomPendingChecks(room).values())if(pending.targetClientId===clientId)sendPendingCheckCommand(conn,pending);}
...
function sendPendingMoveGateCommand(conn,pending){const gate=pending?.gate||{},note=topologyMoveGateNote(gate);conn.sendJSON({type:'gm_command',command:{id:pending.requestId,type:'check_request',targetId:pending.targetClientId,skill:gate.skill,dc:gate.dif,note,gmName:'GM',context:{kind:'vtt_move',gate:gate.kind,tokenId:pending.tokenId},at:now()}});}
function replayPendingMoveGates(room,clientId,conn){if(!conn)return;for(const pending of roomPendingMoves(room).values())if(pending.targetClientId===clientId)sendPendingMoveGateCommand(conn,pending);}
function requestRemoteMoveGate(room,pending,gate){const target=room.members.get(pending.targetClientId);if(!target||target.player.role==='gm'||!target.conn)return false;const pm=roomPendingMoves(room);for(const [rid,p] of pm)if(p.tokenId===pending.tokenId)pm.delete(rid);const requestId=crypto.randomUUID(),note=topologyMoveGateNote(gate);pending={...pending,requestId,mapId:cleanText(room.tactical?.map?.id||'',180),gateKind:gate.kind,gate:jsonClone(gate,{})};pm.set(requestId,pending);sendPendingMoveGateCommand(target.conn,pending);pushSystemLog(room,`Movement pending ${target.player.name}: ${gate.skill} DIF ${gate.dif} · ${note}.`);touch(room);broadcast(room);return true;}
...
function handleRoomMessage(conn, msg, clientId) {
  ...
  switch (msg.type) {
    case 'presence_update': {
      ...
      // Keep Ranger/DMPC labels fresh without stealing initiative authority from the GM.
      if (room.encounter?.actors?.length) {
        const live = role === 'gm' ? member.player.dmpc : member.player;
        const actor = room.encounter.actors.find(a => a.kind === 'player' && a.clientId === clientId);
        if (actor && live) { actor.name = live.name; actor.ranger = live.ranger; room.encounter.updatedAt = now(); }
        else if (actor && role === 'gm' && !live) { syncEncounterPlayers(room); }
      }
    ...
    case 'story_points_set': {
      const value = clampNumber(msg.value || 0, 0, 99, 0);
      const base = nullableNumber(msg.base, 0, 99);
      if (member.player.role !== 'gm' && base !== null) room.storyPoints = clampNumber(room.storyPoints + (value - base), 0, 99, room.storyPoints);
      else room.storyPoints = value;
      touch(room); broadcast(room); break;
    }
    ...
    case 'gm_actor_update': {
      ...
      if (Object.prototype.hasOwnProperty.call(p, 'initiative')) { actor.initiative = nullableNumber(p.initiative); if (actor.kind === 'player') actor._initiativeOwner = 'gm'; }
      if (actor.kind === 'npc') {
    ...
    case 'gm_party_lifecycle': {
      ...
      if (scope === 'session') {
        ...
        room.storyPoints = Math.max(1, [...room.members.values()].reduce((n,m)=>n+(m.conn&&m.player.role!=='gm'?1:m.conn&&m.player.dmpc?1:0),0));
        const e = room.encounter || (room.encounter = blankEncounter());
        e.active = false; e.round = 1; e.turnIndex = -1; for (const a of e.actors) { a.initiative = null; if (a.kind === 'player') a._initiativeOwner = 'player'; } e.updatedAt = now();
        for (const m of room.members.values()) { const p=m.player.role==='gm'?m.player.dmpc:m.player; if(p){ p.initiative=null; p.turnActive=false; p.defending=false; p.updatedAt=now(); } }
    ...
    case 'gm_player_damage': {
      ...
      if (!target || (target.player.role === 'gm' && !target.player.dmpc)) { sendError(conn, 'Target Ranger is not in the room.'); break; }
      if (!target.conn) { sendError(conn, 'Target Ranger is currently offline.'); break; }
      target.conn.sendJSON({ type: 'gm_command', command: { id: crypto.randomUUID(), type: target.player.role === 'gm' ? 'dmpc_damage' : 'damage', targetId, amount } });
      break;
    }
    case 'gm_check_request': {
      ...
      for (const targetId of targets) {
        ...
        const pending={requestId,targetClientId:targetId,skill,dc,note,at:now()};
        roomPendingChecks(room).set(pendingCheckKey(requestId,targetId),pending);
        sendPendingCheckCommand(target.conn,pending);
    ...
    case 'player_check_result': {
      ...
      const requestId=cleanText(r.requestId||'',180),movePending=requestId?roomPendingMoves(room).get(requestId):null,checkKey=pendingCheckKey(requestId,clientId),checkPending=requestId?roomPendingChecks(room).get(checkKey):null;
      if (!requestId || (!movePending && !checkPending)) { sendError(conn, 'That check request is no longer pending.', 'CHECK_REQUEST_EXPIRED'); break; }
      const authoritativeSkill=checkPending?.skill||movePending?.gate?.skill||r.skill||'Skill';
      const authoritativeDc=checkPending?.dc??movePending?.gate?.dif??r.dc??0;
      const skill = cleanText(authoritativeSkill, 80);
      const dc = clampNumber(authoritativeDc, 0, 999, 0);
      const automatic = !!r.automatic;
      const stage = cleanText(r.stage || '', 100);
      const total = nullableNumber(r.total, -9999, 9999);
      const success = automatic ? !!r.success : (total !== null ? total >= dc : !!r.success), critical = !!r.critical, fumble = !!r.fumble;
      ...
      if(checkPending)roomPendingChecks(room).delete(checkKey);
      const authoritativeResult={...r,requestId,skill,dc,total,success};
      if (resolvePendingMoveCheck(room, clientId, authoritativeResult)) break;
      touch(room); broadcast(room); break;
    }
    case 'vtt_map_set': {
      if (!requireGm(conn, room, clientId)) break;
    ...
    case 'gm_reveal_show': {
      if (!requireGm(conn, room, clientId)) break;
      const assetId=cleanAssetId(msg.assetId), asset=room.assets?.get(assetId);
      if(!asset || asset.kind!=='reveal'){sendError(conn,'Reveal image is not available in this room.','REVEAL_ASSET_MISSING');break;}
      const oldSerial=clampNumber(room.reveal?.serial||0,0,Number.MAX_SAFE_INTEGER,0);
      room.reveal={active:true,assetId,name:cleanText(msg.name||asset.name||'Reveal',160),serial:oldSerial+1,updatedAt:now()};
      touch(room);broadcast(room);break;
    }
    case 'gm_reveal_hide': {
      if (!requireGm(conn, room, clientId)) break;
      const oldSerial=clampNumber(room.reveal?.serial||0,0,Number.MAX_SAFE_INTEGER,0);
      room.reveal={active:false,assetId:'',name:'',serial:oldSerial+1,updatedAt:now()};
      touch(room);broadcast(room);break;
    }
    case 'asset_upload': {
    ...
    case 'asset_remove': {
      ...
      if(room.assets?.delete(assetId)){
        const cleared=!!(room.reveal?.active&&room.reveal.assetId===assetId);
        if(cleared) room.reveal={active:false,assetId:'',name:'',serial:(room.reveal.serial||0)+1,updatedAt:now()};
        touch(room);
        if(cleared) broadcast(room);
        broadcastAssetRemove(room,assetId);
      }
      break;
    }
    case 'leave':
...
function makeConnection(socket) {
  const conn = {
    ...
    sendFrame(frame) { if (!this.closed && !socket.destroyed) socket.write(frame); },
...
const server = http.createServer((req, res) => {
  if (req.url === '/health') {
    ...
    res.end(JSON.stringify({ ok: true, protocol: PROTOCOL, rooms: rooms.size, version: '0.33-exp' }));
    return;
  }
...
server.listen(PORT, HOST, () => console.log(`Power Rangers RPG v0.33-exp GM Session Flexibility & Reveal Board room server listening on ws://${HOST}:${PORT}`));
