...
/**
 * Power Rangers RPG Character Builder — v0.31.10-exp Final Integration & Release Readiness audit room server
  ...
  switch (msg.type) {
    ...
    case 'gm_party_lifecycle': {
      ...
      if (scope === 'session') {
        ...
        room.storyPoints = Math.max(1, [...room.members.values()].filter(m => m.player.role !== 'gm' && !!m.conn).length);
        const e = room.encounter || (room.encounter = blankEncounter());
        ...
        for (const m of room.members.values()) if (m.player.role !== 'gm') { m.player.initiative = null; m.player.turnActive = false; m.player.defending = false; m.player.updatedAt = now(); }
...
const server = http.createServer((req, res) => {
  if (req.url === '/health') {
    ...
    res.end(JSON.stringify({ ok: true, protocol: PROTOCOL, rooms: rooms.size, version: '0.31.10-exp' }));
    return;
  }
...
server.listen(PORT, HOST, () => console.log(`Power Rangers RPG v0.31.10-exp Final Integration & Release Readiness audit room server listening on ws://${HOST}:${PORT}`));
